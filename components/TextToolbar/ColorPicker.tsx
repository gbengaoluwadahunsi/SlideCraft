...
export const ColorPicker: React.FC<ColorPickerProps> = ({ isOpen, onClose, onToggle }) => {
  const { execCommand, recentColors, saveSelection } = useTextToolbar();
  ...
  useEffect(() => {
    if (isOpen && buttonRef.current) {
      ...
      setPos({
        top: rect.bottom + 8,
        left: Math.max(0, rect.left + rect.width / 2 - 104),
      });
    }
  }, [isOpen]);
  ...
  const dropdown = (
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          className="fixed z-[9999] bg-gray-800 border border-gray-700 rounded-xl shadow-2xl p-3 w-52"
